...
class MyPlayer(PlayerAbalone):
    ...
    def evaluate_state(self, state:GameStateAbalone, debug=False):
        ...
        other_central_score = self.distance_to_center(other_player_pieces)
        ...
        if debug:
            ...
            print("Score piece:", score_pieces)
            print("Central Score:", central_score)
            print("Central Score (Other player):", other_central_score)
        ...
        if central_score > 0.5:
            ...
            a, b, c, d, e = 10, 1, 5, 0.1, 0.05  # Poids initiaux
                
        # Phase 2 : Groupe Compact + Dominance Centrale
        elif connectedness_score < 0.5:
            if debug:
                print("Phase 2 : Groupe Compact + Dominance Centrale")
            a, b, c, d, e = 10, 0.5, 1, 2, 0.05  # Poids ajustés pour la compacité et le contrôle central
            
        # Phase 3 : Jeu Agressif
        else:
            if debug:
                print("Phase 3 : Jeu Agressif")
            a, b, c, d, e = 10, 0, 0.1, 0, 1  # Changement de focus vers un jeu plus agressif
            if ejected_pieces >= 3:  # Aggression accrue après l'éjection de 5 pièces
                e = 3  # Augmentation des poids pour l'agression

        score = a * ball_score + b * score_pieces - c * central_score + d * connectedness_score - e * connectedness_score_other_player 
        if debug:
